...
export const createTemplateV2 = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!identity?.subject)
      throw new Error("createTemplateV2: unauthenticated");
    ...
    if (typeLower === "monster") {
      const atkOk =
        args.atkPts === undefined ||
        (Number.isFinite(args.atkPts) &&
          args.atkPts >= 0 &&
          args.atkPts <= 5000);
      const defOk =
        args.defPts === undefined ||
        (Number.isFinite(args.defPts) &&
          args.defPts >= 0 &&
          args.defPts <= 5000);
      const lvlOk =
        args.level === undefined ||
        (Number.isFinite(args.level) && args.level >= 0 && args.level <= 10);
      if (!atkOk || !defOk || !lvlOk)
...
export const getDictionaryTemplates = query({
  ...
  handler: async (ctx) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity?.subject)
      throw new Error("getDictionaryTemplates: unauthenticated");
    ...
    if (me.serverId) {
      ...
      if (templateIds.length > 0) {
        const fetched: Array<Doc<"card_templates"> | null> = await Promise.all(
          templateIds.map(async (tidStr) =>
            ctx.db.get(tidStr as unknown as Id<"card_templates">)
          )
        );
        serverWorkshopTemplates = fetched.filter(
          (t): t is Doc<"card_templates"> => !!t
        );
      }
    }
  ...
  handler: async (ctx) => {
    ...
    const userCards = await ctx.db
      ...
      .withIndex("by_server_user", (q) =>
        q.eq("serverId", me.serverId!).eq("userId", me._id)
      )
      .collect();
      ...
      userCards.map(async (uc) => {
        ...
        const listings = await ctx.db
          ...
          .withIndex("by_server_status", (q) =>
            q.eq("serverId", uc.serverId).eq("status", "active")
          )
          .collect();
...
export const getServerListingsV2 = query({
  args: {
    scope: v.optional(
      v.union(v.literal("active"), v.literal("sold"), v.literal("cancelled"))
    ),
  ...
  handler: async (ctx, { scope = "active", limit = 50 }) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity?.subject)
      throw new Error("getServerListingsV2: unauthenticated");
    ...
    const listings: Doc<"listings">[] = await ctx.db
      .query("listings")
      .withIndex("by_server_status", (q) =>
        q.eq("serverId", me.serverId as Id<"servers">).eq("status", scope)
      )
      .order("desc")
    ...
    const results = await Promise.all(
      listings.map(async (l: Doc<"listings">) => {
        ...
        const templateId: Id<"card_templates"> | undefined =
          userCard?.cardTemplateId as Id<"card_templates"> | undefined;
        const template: Doc<"card_templates"> | null = templateId
          ? await ctx.db.get(templateId)
          : null;
        const sellerName: string | null = seller
          ? (seller.username ?? seller.name ?? "Player")
          : null;
        const category: "trade" | "sale" =
          Number(l.price ?? 0) <= 1 ? "trade" : "sale";
        return {
          _id: l._id,
...
export const createListingV2 = mutation({
  ...
  handler: async (ctx, { userCardId, price }) => {
    ...
    if (!Number.isFinite(price) || price <= 0)
      throw new Error("createListingV2: invalid price");
    ...
    const userCard: Doc<"user_cards"> | null = await ctx.db.get(
      userCardId as Id<"user_cards">
    );
    if (!userCard) throw new Error("createListingV2: user card not found");
    if (String(userCard.userId) !== String(me._id))
      throw new Error("createListingV2: not owner");
    if (String(userCard.serverId) !== String(me.serverId))
      throw new Error("createListingV2: cross-server listing denied");
    if (!Number.isFinite(userCard.quantity) || userCard.quantity <= 0)
      throw new Error("createListingV2: no quantity to list");

    // Prevent duplicate active listings for the same userCardId by the same seller
    const dup = await ctx.db
      .query("listings")
      .withIndex("by_userCard_status", (q) =>
        q
          .eq("userCardId", userCardId as Id<"user_cards">)
          .eq("status", "active")
      )
      .first();
...
export const purchaseListingV2 = mutation({
  ...
  handler: async (ctx, { listingId }) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity?.subject)
      throw new Error("purchaseListingV2: unauthenticated");
    const me: Doc<"users"> | null = await ctx.db
      .query("users")
      .withIndex("by_clerk_id", (q) => q.eq("clerkId", identity.subject))
      .first();
    ...
    const listing: Doc<"listings"> | null = await ctx.db.get(
      listingId as Id<"listings">
    );
    if (!listing) throw new Error("purchaseListingV2: listing not found");
    if (listing.status !== "active")
      throw new Error("purchaseListingV2: listing not active");
    if (String(listing.serverId) !== String(me.serverId))
      throw new Error("purchaseListingV2: cross-server purchase denied");
    if (String(listing.sellerId) === String(me._id))
      throw new Error("purchaseListingV2: cannot buy own listing");

    const [seller, userCard]: [Doc<"users"> | null, Doc<"user_cards"> | null] =
      await Promise.all([
        ctx.db.get(listing.sellerId as Id<"users">),
        ctx.db.get(listing.userCardId as Id<"user_cards">),
      ]);
    if (!seller || !userCard)
      throw new Error("purchaseListingV2: seller or user card missing");
    if (me.goldCount < listing.price)
      throw new Error("purchaseListingV2: insufficient gold");

    // Transfer: decrement seller quantity, increment or create buyer user_card
    const templateId: Id<"card_templates"> =
      userCard.cardTemplateId as Id<"card_templates">;
    const buyerExisting: Doc<"user_cards">[] = await ctx.db
      .query("user_cards")
      .withIndex("by_server_user", (q) =>
        q
          .eq("serverId", me.serverId as Id<"servers">)
          .eq("userId", me._id as Id<"users">)
      )
      .collect();
    const sameTemplate = buyerExisting.find(
    ...
    await Promise.all([
      ctx.db.patch(me._id, {
        ...
        highestGoldCount: Math.max(
          me.highestGoldCount,
          me.goldCount - listing.price
        ),
        currentCardCount: me.currentCardCount + 1,
        highestCardCount: Math.max(
          me.highestCardCount,
          me.currentCardCount + 1
        ),
      ...
      ctx.db.patch(seller._id, {
        ...
        highestGoldCount: Math.max(
          seller.highestGoldCount,
          seller.goldCount + listing.price
        ),
      ...
      ctx.db.patch(userCard._id as Id<"user_cards">, {
        quantity: Math.max(0, (userCard.quantity ?? 0) - 1),
      }),
      (async () => {
        if (sameTemplate) {
          await ctx.db.patch(sameTemplate._id as Id<"user_cards">, {
            quantity: (sameTemplate.quantity ?? 0) + 1,
          });
        } else {
...
export const unlistListingV2 = mutation({
  ...
  handler: async (ctx, { listingId }) => {
    ...
    if (String(listing.sellerId) !== String(me._id))
      throw new Error("unlistListingV2: not your listing");
